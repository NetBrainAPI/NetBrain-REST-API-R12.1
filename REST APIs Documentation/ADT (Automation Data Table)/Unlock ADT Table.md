...
|password*|string| This parameter is only required when `lockMode = 1` from Lock ADT Table API, i.e. ADT Table is locked.<br> Note: The following situations do not require a password: 1) The user owns the table, 2) The user is a domain admin, 3) Table is Locked Without Password (`lockMode=0` in Lock ADT Table API)  |
